#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

echo
echo "================================================"
echo "          CYBERVOYRAX Workspace Setup"
echo "================================================"
echo
echo "WARNING:"
echo "CYBERVOYRAX is intentionally vulnerable."
echo "Run it only in an authorized/local lab environment."
echo "Do not expose it directly to the public internet."
echo

bash scripts/check_requirements.sh

echo
echo "[*] Building and starting CYBERVOYRAX..."
echo

docker compose up -d --build

APP_PORT_VALUE="${APP_PORT:-5000}"
if [[ -f ".env" ]]; then
    ENV_PORT="$(sed -n 's/^APP_PORT=//p' .env | tail -n1)"
    if [[ -n "$ENV_PORT" ]]; then
        APP_PORT_VALUE="$ENV_PORT"
    fi
fi

echo
echo "[*] Waiting for CYBERVOYRAX to finish initialization..."

APP_URL="http://127.0.0.1:${APP_PORT_VALUE}/login"

for i in {1..90}; do
    if curl --fail --silent         --max-time 3         "$APP_URL"         >/dev/null 2>&1; then

        echo "[✓] Application is ready"
        break
    fi

    if ! docker compose ps --status running web 2>/dev/null | grep -q web; then
        echo
        echo "[✗] Web container stopped during initialization."
        echo
        docker compose logs --tail=150 web
        exit 1
    fi

    if [[ "$i" -eq 90 ]]; then
        echo
        echo "[✗] Application did not become ready within 3 minutes."
        echo
        docker compose logs --tail=150 web
        exit 1
    fi

    sleep 2
done

echo
echo "[*] Provisioning assessment account..."
bash scripts/provision_lab_accounts.sh

echo
echo "[*] Verifying account baseline..."
docker compose exec -T web python scripts/verify_account_baseline.py

echo "================================================"
echo "          CYBERVOYRAX is ready"
echo "================================================"
echo
echo "URL:"
echo "  http://127.0.0.1:${APP_PORT_VALUE}"
echo
if [[ -f ".lab-credentials" ]]; then
    echo "Assessment credentials:"
    echo "  .lab-credentials"
    echo
    echo "View them with:"
    echo "  cat .lab-credentials"
else
    echo "Assessment account:"
    echo "  Existing account preserved."
    echo "  No local .lab-credentials file is available."
fi
echo
